...
### Consul 与 Eureka
Eureka 是一个服务发现工具，它的架构主要由客户端/服务器组成。每个数据中心有一组 Eureka 服务，通常是一个可用区域一个。通常，Eureka 客户端通过内置的 SDK 去注册和发现服务，而对于非内部集成的客户端，会使用 Ribbon 等方式向 Eureka 查询服务。  
Eureka 使用复制来提供弱一致性的服务。当客户端向服务器注册时，该服务器会尝试将数据复制给其他服务器，但是并不保证成功。服务注册只有一个短暂的存活时间(TTL)，还需要客户端想服务器发送心跳信息。不健康的服务或者节点不会发送心跳信息，注册中心就会把它们移除服务列表。基于复制的协议，服务发现请求可能会被路由给老的服务方。使用这种简单的模型可以很容易地进行集群管理和维持集群高扩展性。  
Consul 提供了更多的功能，包括更丰富的健康检查，key/value 存储和多数据中心
